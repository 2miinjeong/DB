...
def fetch_series_images(series_id, language='ko'):
    ...
    # 포스터 이미지가 여러개인 경우 첫 번째 이미지만 가져오도록 설정
    if 'posters' in data and len(data['posters']) > 0:
        data['posters'] = data['posters'][:1]

    # 로고 이미지가 여러개인 경우 첫 번째 이미지만 가져오도록 설정
...
def insert_series_to_database(series, series_images, series_videos):
    ...
    #SQL INSERT
    sql = "INSERT INTO KIDS1 (SERIES_ID, SUB_CATEGORY, TITLE, GENRE, SERIES_RATING, SEASON_NUM, SERIES_OVERVIEW, CAST, CREW, LOGO, POSTER, TRAILER) VALUES (%s, '시리즈', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
